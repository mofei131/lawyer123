<template name="gui-im-input">
	<view>
		...
		<view class="gui-im-record" v-if="recShow">
			...
			<view class="gui-im-send-voice" v-if="tmpVoice != ''"><text @tap="sendVoiceMsg">发送语音</text></view>
...
export default {
	...
	created: function() {
		...
		this.recorderManager.onError(() => {
			...
			this.recing = false;
			(this.recTxt = '请点击绿色按钮开始录音'), clearInterval(this.recTimer);
		});
		// #endif
		// #ifdef MP
		try {
...
.graceIMFonts {
	font-family: 'graceIMFonts' !important;
	font-style: normal;
	color: #2b2e3d;
	font-size: 56rpx;
}
